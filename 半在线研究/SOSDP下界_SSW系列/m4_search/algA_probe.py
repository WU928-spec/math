...
random.seed(seed)
heaps = {m: [] for m in (3,4,5,6,8)}   # min-heap of (cheap_ratio, jobs)
...
for _ in range(N):
    ...
    h = heaps[m]
    if len(h) < 100:
        heapq.heappush(h, (cr, jobs))
    elif cr > h[0][0]:
        heapq.heapreplace(h, (cr, jobs))

out = open(f"/mnt/agents/output/m4_search/algA_probe_{seed}.txt","w")
for m in (3,4,5,6,8):
    cands = sorted(heaps[m], key=lambda x:-x[0])[:30]
    besttrue = 0; bestjobs=None; ba=bo=0
    for cr, jobs in cands:
        o = opt_jobs(jobs, m); a = algA(jobs, m)
        if a/o > besttrue:
            besttrue = a/o; bestjobs = jobs; ba, bo = a, o
    out.write(f"m={m}: TRUE max ratio = {besttrue:.8f}  algA={ba:.5f} opt={bo:.5f}\n  jobs={bestjobs}\n")
out.close()
print("done", seed)
