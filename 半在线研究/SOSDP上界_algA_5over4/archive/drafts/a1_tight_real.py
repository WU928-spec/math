...
def realizable(s, j, p, q1, K):
    ...
    nS = len(s)
    ks = [i for i in range(nS) if s[i] <= K - q1 + 1e-9]
    k = len(ks)
    if k == 0 or k > nS:
        return False, k
    jmax = j[-1]
    #  senior i (0-based) 低端条件 i<k；机 jj=k-1 须取 jmax（fs）
    # DFS: seniors 从大到小指派（更易剪枝）；低端约束转置后处理复杂，改从小到大。
    used = [False] * nS
    assign = [None] * nS
    ...
    def dfs(i, prev_j):
        ...
        lo = 0
        if i < k:
            lo = prev_j  # mon2: junior 值不减（以值域下标表达）
        for jj in range(lo, nS):
            ...
            if sm > K + 1e-9:
                continue
            used[jj] = True; assign[i] = jj
            if dfs(i + 1, jj):
                return True
            used[jj] = False; assign[i] = None
        return False
