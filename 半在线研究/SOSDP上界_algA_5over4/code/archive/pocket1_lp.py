"""口袋1子情形B（最闲机2件M0={x,y}，所有2件台含大任务）LP + 精确常数证书。
结构同 farkas_constant：t 只在右端 -> 常数证书覆盖全 t 窗口。
简化装箱：体积 <= m（聚合）；senior/y 大任务不进3件箱由 s_i>1-2t 体现。
"""
import numpy as np
...
def build_p1(m, k=1):
    ...
    for i in range(nS):
        ...
        r = zero(); r[ix] = 1; r[iy] = 1; r[vs(i)] = -1; r[vj(i)] = -1; con(r, -MG, 0, f'mach{i}>M0')
...
def rationalize_verify(A, bc, bt, yf):
    for N in [10**3, 10**4, 10**5, 10**6, 10**7]:
        ...
        if any(A[i][j] * y[i] != 0 for j in range(len(A[0])) for i in range(len(y))):
            # 太慢，改用逐变量累加
            pass
        ok = True
...
if __name__ == '__main__':
    print('口袋1子情形B（2件台全含大任务）常数证书扫描（m × k）:')
    for m in range(4, 9):
        row = []
        for k in range(1, m):
            A, bc, bt, names, nv = build_p1(m, k)
